...
train_stage=-10
get_egs_stage=-10
affix=_1a_ep3_dim850  # 2 layers have dropout with proportion 0.2
decode_iter=
lat_beam=6.5
beam=15.0
...
#    --feat.online-ivector-dir exp/nnet3/ivectors_${train_set} \
  ...
  steps/nnet3/chain/train_e2e.py --stage $train_stage \
    ...
    --trainer.options="--prune=$prune --prune-beam=$prune_beam --num-scale=$num_scale --offset-first-transitions=$normalize_egs --den-use-initials=$den_use_initials --den-use-finals=$den_use_finals --check-derivs=$dbl_chk" \
...
~/bin/swbd.sh $dir >> biphone_runs.log
