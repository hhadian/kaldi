...
static bool ProcessFile(const TransitionModel *trans_mdl,
  ...
  for (size_t c = 0; c < chunks.size(); c++) {
    ...
    std::ofstream of;
    of.open("chunk" + std::to_string(c) + ".txt");
    fst::RmEpsilon(&(supervision_part.fst));
    WriteFstKaldi(of, false, supervision_part.fst);
    of.close();
    ...
    //std::ofstream of;
    if (supervision_part.e2e_fsts.size() == 1) {
      of.open("chunk" + std::to_string(c) + "-UC.txt");
      fst::RmEpsilon(&(supervision_part.fst));
      WriteFstKaldi(of, false, supervision_part.e2e_fsts[0]);
      of.close();
    }
